...
export class ScreenBelowLargeDirective implements OnDestroy {
  ...
  constructor(
    ...
    this.screenSubscription = screenService.resize$.subscribe(() =>
      this.onResize()
    ); // Fires onResize() as onNext() handler for when resize$ emits its next() trigger.
  }
  ...
  set screenBelowLarge(condition) {
    ...
    condition =
      this.screenService.screenWidth < this.screenService.largeBreakPoint; // This is the only line of code that differs from scree-large.directive.
