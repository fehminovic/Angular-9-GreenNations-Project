...
export class UserService implements UserApi {
  ...
  signIn(
    ...
    console.log(
      "UserService.signIn: " + username + " " + password + " " + rememberMe
    );
    ...
    if (!this.isFakeError) {
      ...
      if (!this.isFakeDelay) {
        return of({}); // SUCCESS: Returns empty reservable.
      } else {
        return of({}).pipe(delay(this.fakeDelay)); // Returns empty reservable.  Adding delay for actual api request/response effect
      }
    } else {
      ...
      if (this.isFakeDelay) {
        ...
        return of({})
          .pipe(delay(this.fakeDelay))
          .pipe(
            flatMap((x) =>
              Observable.throw("Invalid User Name and/or Password")
            )
          );
      } else {
  ...
  signOut(): Observable<any> {
    ...
    return of({}); // Returns empty reservable
  }
}
